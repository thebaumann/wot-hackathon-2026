      ...
      <Transition name="audience-fade" mode="out-in">
        <div :key="audience" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            ...
            :key="i"
            class="bg-gray-800 rounded-xl p-6 flex flex-col gap-4 border border-gray-700 hover:border-trumpf-blue/40 transition-colors duration-200"
          >
            <!-- Quote mark -->
            <svg class="w-8 h-8 text-trumpf-blue opacity-60 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="pt-3 border-t border-gray-700">
              ...
              <p class="text-trumpf-blue text-xs font-medium mt-0.5">{{ t_.company }}</p>
            </div>
          </div>
        </div>
      </Transition>
